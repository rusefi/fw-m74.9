# M74.9 technical details

Build, memory-contract, tooling and Java UI internals for the M74.9 rusEFI
port. End-user installation and update instructions are in [readme.md](readme.md).
...
No response leaves the firmware status unknown. Generic rusEFI identity alone
does not establish M74.9 update support, so that state disables flashing.
...
reports PCAN presence even if the ECU does not respond. Unplugging and
reconnecting the adapter permits a new automatic query.
...
activation ABI; calibration uses a separate `--calibration` upload.
See [the uploader guide](docs/cli-uploader.md) for the required metadata handshake,
verification choices, exit codes and pending hardware validation, and
[the flash-reader guide](docs/cli-flash-reader.md) for the RAM-helper backup
transaction, resume behavior and adapter pacing options.
